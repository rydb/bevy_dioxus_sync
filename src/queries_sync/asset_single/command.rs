...
use crate::{add_systems_through_world, queries_sync::asset_single::AssetWithHandle, BevyRxChannel, BevyTxChannel};
...
fn send_asset_singleton<T, U, V>(
    ...
    where
        ...
        V: Component,
{
    let Ok((e, handle, _)) = handle.single()
    ...
    bevy_tx.0.send(AssetWithHandle { asset: asset.clone(), handle: handle, new_type: PhantomData::default()});
}
