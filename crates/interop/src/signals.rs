use std::{any::type_name, cell::OnceCell, collections::HashSet, fmt::{Debug, Display}, sync::{Arc, LazyLock, Mutex, MutexGuard, OnceLock, PoisonError, TryLockError, atomic::AtomicPtr}};
...
impl<T> Default for CrossDomSignal<T> {
    fn default() -> Self {
        Self { value: Arc::new(OnceLock::new()), subscribers: Default::default() }
    }
}
...
impl Display for SetError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let erorr = match self {
            ...
            SetError::SignalFetchError(signal_fetch_error) => format!("signal fetch error: {}", signal_fetch_error),
...
impl<T> CrossDomSignal<T> {
    pub fn get(&self) -> Result<Arc<T>, SignalFetchError> {
        ...
        match ReactiveContext::current() {
            ...
            None => {
                warn!("no reactive context found?");
        ...
        match self.value.get() {
            Some(value) => {
                ...
                Ok(value.as_ref().load().clone())
            },
            None => {
                warn!("LOAD UNSUCCESSFUL");
                Err(SignalFetchError::Uninitialized)
            },
        }
    }
    ...
    pub fn new(value: T) -> Self {
        ...
        let _ = once.set(Arc::new(ArcSwap::new(value.into()))).inspect_err(|_err| warn!("Initializing OnceLock failed for {}. How did this happen?", type_name::<T>()));
        Self {
            value: Arc::new(once),
    ...
    pub fn pnt_to(&self, ptr: Arc<ArcSwap<T>>) {
        let _ = self.value.set(ptr).inspect_err(|_err| warn!("initialize failed for {}", type_name::<T>()));
    } 
    pub fn initialize(&self, value: T) {
        let _ = self.value.set(ArcSwap::new(value.into()).into()).inspect_err(|_err| warn!("initialize failed for {}", type_name::<T>()));
    } 
