...
impl<T: Component + Clone> Command for RequestBevyComponents<T> {
    fn apply(self, world: &mut World) -> () {
        ...
        let component_hook =
            world
                ...
                .try_on_remove(|mut world, hook| {
                    let bevy_tx = world
                        .get_resource_mut::<BevyTxChannel<EntityComponentQueue<T>>>()
                        .unwrap();
                    ...
                    let _ = bevy_tx.0.send(new_requests).inspect_err(|err| {
                        warn!(
                            "could not send remove request for {:#} for {:#} due to {:#}",
                            hook.entity,
                            type_name::<T>(),
                            err
                        )
        ...
        if component_hook.is_none() {
            warn!(
                "could not add .on_remove hook for {:#} because it already has one, using manual system check instead",
                type_name::<T>()
            );
...
// fn populate_initial_entity_component_map<T: Component + Clone>(
